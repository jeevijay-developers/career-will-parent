...
import { getFeesByRollNumber, FeeRecord, FeeSubmission } from "../util/server";
...
const FeeSection: React.FC = () => {
  ...
  const fee = useMemo<FeeRecord | undefined>(() => (fees && fees.length > 0 ? fees[0] : undefined), [fees]);

  const allSubmissions = useMemo<FeeSubmission[]>(() => {
    const list: FeeSubmission[] = fee?.submissions || [];
    let filtered = list;
    if (dateRange.startDate || dateRange.endDate) {
      ...
      filtered = list.filter((s) => {
        const sDate = new Date((s as any).dateOfReceipt || (s as any).date);
        return sDate >= start && sDate <= end;
      });
    }
    return [...filtered].sort(
      (a, b) => new Date((b as any).dateOfReceipt || (b as any).date).getTime() - new Date((a as any).dateOfReceipt || (a as any).date).getTime()
    );
  }, [fee, dateRange]);
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <div className="text-center p-3 bg-gray-50 rounded-lg">
          ...
          <div className="text-lg font-semibold text-gray-900">{currency(fee.finalFees || (fee as any).totalFees)}</div>
        </div>
        <div className="text-center p-3 bg-gray-50 rounded-lg">
        ...
        {allSubmissions.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-200">
            {allSubmissions.map((s, idx) => (
              <li key={(s as any)._id || idx} className="p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center">
                    <CreditCard className="w-4 h-4 text-gray-600" />
                  </div>
                  <div>
                    <div className="text-sm font-medium text-gray-900">{currency((s as any).amount)}</div>
                    <div className="text-xs text-gray-600 flex items-center gap-2">
                      <span>{(s as any).mode}</span>
                      <span>•</span>
                      <span>Receipt #{(s as any).receiptNumber}</span>
                      {(s as any).UTR && (
                        <>
                          <span>•</span>
                          <span>UTR: {(s as any).UTR}</span>
                        </>
                      )}
                    </div>
                  </div>
                </div>
                <div className="text-xs text-gray-600">{formatDate((s as any).dateOfReceipt || (s as any).date)}</div>
              </li>
            ))}
          </ul>
        )}
